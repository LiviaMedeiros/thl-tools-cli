...
Run `./thl-tools-cli.exe extract "[PATH_TO_THE_GAME_FILE]/gamedata/app_0.dx11.mvgl" extracted-app-0`  
...
If you're playing the game in English, run `./thl-tools-cli.exe pack modified-files [PATH_TO_THE_GAME_FILE]/gamedata/patch_1.dx11.mvgl`. That way, you'll only repack the files you modified, so it'll take less time. (if you're playing in Japanese, replace `patch_1` by `patch_0`, or by `2` if in Traditionnal Chinese, and `3` if in Simplified Chinese).
...
Run `./thl-tools-cli.exe extract-dialogues [PATH_TO_THE_GAME_FILES] [languages_you_want_to_extract]`  
For instance, if you want the text in both Japanese and English, run `./thl-tools-cli.exe extract-dialogues "C:\Program Files (x86)\Steam\steamapps\common\The Hundred Line -Last Defense Academy-" japanese,english`
...
Then, run `./thl-tools-cli.exe repack-dialogues full-text.csv [PATH_TO_THE_GAME_FILES] [REFERENCE_LANGUAGE]`. The reference language is the language the untranslated line will be.
For instance, to apply your patch on the English translation (meaning any untranslated line will remain in English), you'll want to run something like `./thl-tools-cli.exe repack-dialogues full-text.csv "C:\Program Files (x86)\Steam\steamapps\common\The Hundred Line -Last Defense Academy-" english`
